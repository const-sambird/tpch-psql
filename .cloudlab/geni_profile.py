...
node_index_tuner.addService(pg.Execute(shell='bash', command='sudo chmod a+w /opt/tpch-psql'))
...
node_index_tuner.addService(pg.Execute(shell='bash', command='cat >/opt/tpch-psql/config.csv <<EOL\n' + parsed_config + '\nEOL'))
node_index_tuner.addService(pg.Execute(shell='bash', command='cp -r /proj/qdina-PG0/tpc-h /opt/tpch-psql'))
node_index_tuner.addService(pg.Execute(shell='bash', command='sudo chown -R sambird:qdina-PG0 /opt/tpch-psql/tpc-h'))
